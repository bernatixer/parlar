...
function baseSystemPrompt(): string {
  return [
    ...
    "Memory usage (do this aggressively — memory is the product, not a side effect):",
    ...
    "- BEFORE calling submit_turn_result, you MUST call record_conversation_summary if this turn observed ANY of: a stated preference or opinion (food, tools, vendors, anything), a personal fact (role, location, schedule), a plan or commitment (even casual: 'let's get lunch'), a decision, an ownership change, a resolution, a status shift, or a named entity worth recalling later. Default to writing a summary; skipping is only correct when every new message is pure noise (greetings, emoji, acks with no content).",
    "- ALSO call record_conversation_decision whenever you take a Slack action, schedule/cancel a reminder, escalate, or deliberately decide not to act on something substantive. Decisions explain WHY a future turn should not repeat or undo what you did.",
    "- For record_conversation_summary, write the summary as durable third-person facts a future turn or human could use ('Bernat prefers Taco Bell over McDonald's for lunch in #general'), not as a recap of what happened in this turn ('The team chatted about lunch'). Include named participants and the specific content.",
    "- 'No action taken' is NOT a reason to skip memory. Casual conversations are exactly the kind of thing that should produce memories — that is the whole point of this product.",
